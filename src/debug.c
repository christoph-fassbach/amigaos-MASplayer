...
VOID debug_fprintf( STRPTR format, ... ) {
  ...
  STRPTR logFilePath = "ram:MAS-MHI.log";
  UBYTE buffer[512];
  ...
  if ( !AmiGUS_MHI_Base->agb_LogFile ) {
    ...
    if ( 36 <= (( struct Library *) DOSBase )->lib_Version ) {
      ...
      LONG i = GetVar( "MAS-MHI-LOG-FILEPATH",
                       buffer,
                       sizeof( buffer ),
                       0 );
      if (( i > 0 ) && ( i < 512 )) {
...
VOID debug_mprintf( STRPTR format, ... ) {
  ...
  if ( !AmiGUS_MHI_Base->agb_LogMem ) {
    ...
    if ( 36 <= (( struct Library * ) DOSBase )->lib_Version) {
      ...
      UBYTE buffer[ 64 ];
      LONG i = GetVar( "MAS-MHI-LOG-ADDRESS", buffer, sizeof( buffer ), 0 );
      /*
       * UAE:  setenv MAS-MHI-LOG-ADDRESS 1207959552 -> 0x48000000
       * 3/4k: setenv MAS-MHI-LOG-ADDRESS 167772160  -> 0x0a000000
       * 2k:   setenv MAS-MHI-LOG-ADDRESS 4194304    -> 0x00400000
       */
      ...
      i = GetVar( "MAS-MHI-LOG-SIZE", buffer, sizeof( buffer ), 0 );
      if ( i > 0 ) {
    ...
    debug_kprintf( "MAS-MHI-LOG-ADDRESS %lx = %ld (requested)\n"
                   "MAS-MHI-LOG-SIZE %ld\n",
                   ( LONG ) where,
                   ( LONG ) where,
                   size );
    ...
    if ( !AmiGUS_MHI_Base->agb_LogMem ) {
      ...
      debug_kprintf( "MAS Log giving up...\n" );
      errorShown = TRUE;
      DisplayError( EAllocateLogMem );
      return;
    }
    debug_kprintf( "MAS Log @ 0x%08lx = %ld (retrieved), size %ld\n",
